$(function () {
  if (jQuery.browser.mobile === false) {
    $(".profile-text", "#profile")
      .find("p:not(:first)")
      .css("opacity", 0)
      .one("inview", function (isInView) {
        if (isInView) {
          $(this).addClass("animated bounceInUp").css("opacity", 1);
        }
      });
    $(".inner-content", "#qualities")
      .find("h4")
      .css("opacity", 0)
      .one("inview", function (isInView) {
        if (isInView) {
    ...
    $(".legend-right", "#strengths")
      .find(".item")
      .each(function (i) {
        var element = $(this),
          itemsDelay = isNaN($(this).data("animation-delay"))
            ? 50
